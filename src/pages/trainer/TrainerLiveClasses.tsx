...
import {
  ...
  Trash2, X, AlertCircle, Sparkles, BookOpen, ShieldCheck
} from "lucide-react";
  ...
  const handleBroadcastInstantMeet = (e: React.FormEvent) => {
    ...
    if (!meetDetails.isReal) {
      addToast({
        ...
        message: "Please paste your Google Meet link (e.g. meet.google.com/abc-defg-hij) so enrolled students can join your live class.",
        type: "error"
      });
      return;
    }
    ...
    addToast({
      ...
      message: `"${newSession.title}" is now LIVE! Room Code: ${newSession.meetingCode}. All enrolled students alerted with auto-joining buzzer.`,
      type: "success"
    });
  };
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              <button
                ...
                className="apple-btn-primary text-xs px-4 py-3 font-bold flex items-center gap-2 shadow-xl shadow-rose-600/30 bg-gradient-to-r from-rose-600 via-red-600 to-rose-700 hover:brightness-110 cursor-pointer text-white transition transform hover:scale-[1.02]"
                title="Go live and broadcast Google Meet to enrolled students"
              >
                <Radio className="w-4 h-4 text-white animate-pulse" /> Go Live (Google Meet)
              </button>
              <button
                  ...
                  <div className="pt-3 border-t border-white/10 flex flex-wrap items-center justify-between gap-3 text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="apple-btn-danger text-xs px-3 py-1.5 font-bold cursor-pointer"
                      >
                        End Class
                      </button>
                    </div>
            ...
            <form onSubmit={handleScheduleSubmit} className="space-y-4">
              ...
              {platform === "google-meet" && (
                <div className="space-y-1.5 p-3 rounded-xl bg-blue-950/20 border border-blue-500/20">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-semibold text-white">Google Meet Room Code (Optional)</label>
                  ...
                  <p className="text-[10px] text-slate-300 leading-tight">
                    Dedicated meeting room is auto-created by Capacity Connect. You do not need to open Google Meet or leave the portal.
                  </p>
                </div>
              )}
